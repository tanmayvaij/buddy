...
export default function SinglePost() {
    ...
    getDocs(q)
      .then((postDoc) => {
        postDoc.forEach((post) =>
          setSinglePost({ ...post.data(), docID: post.id })
        );
      })
      .catch((err) => {
        console.log(err);
  ...
  return (
    <div
      id="singleblog"
      className="items-center min-h-[calc(100vh-57.6px)] justify-center flex flex-col"
    >
      <div className="bg flex items-center justify-center flex-col pb-4 w-full mb-4 border-b-2 border-gray-200">
        <img
          className="rounded-lg m-3 h-96 w-[600px]"
          src={singlePost?.image ?? "/placeholder.webp"}
          alt=""
        />
      ...
      {user.photoURL == singlePost.user_photo ? (
        <DeleteBar doc_id={singlePost.docID} />
      ) : (
        <></>
      )}
    </div>
