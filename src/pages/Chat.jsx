...
export default function Chat() {
  ...
  const getReceiverDetails = async (receiver) => {
    const res = await fetch(
      `https://iron-envelope-379712-default-rtdb.firebaseio.com/users/${receiver}.json`,
      {
        method: "GET",
        headers: {
  ...
  return (
    <div id="chat" className=" min-h-[calc(100vh-57.3px)]">
      ...
      <div
        id="chatsection"
        className="overflow-y-scroll h-[calc(100vh-156.5px)]"
      >
        {chats.map((chat, id) => {
          return (
            <div
              key={id}
              className={`${
                chat.email == email ? "flex-row-reverse" : ""
              } overflow-y-scroll px-5 py-2 flex items-center justify-between`}
            >
      ...
      <div className="flex">
        ...
        <button
          onClick={sendMessage}
          className="text-white rounded-sm px-5 bg-blue-700"
        >
